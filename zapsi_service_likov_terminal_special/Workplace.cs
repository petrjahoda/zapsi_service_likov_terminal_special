...
namespace zapsi_service_likov_terminal_special {
    ...
    public class Workplace {
        ...
        public DateTime GetOrderStartDateTimeFor(ILogger logger) {
            var orderStartDateTime = DateTime.Now;
            var connection = new MySqlConnection($"server={Program.IpAddress};port={Program.Port};userid={Program.Login};password={Program.Password};database={Program.Database};");
            try {
                connection.Open();
                var selectQuery = $"SELECT * from zapsi2.terminal_input_order where DTE is NULL and DeviceID={DeviceOid}";
                ...
                try {
                    var reader = command.ExecuteReader();
                    if (reader.Read()) {
                        orderStartDateTime = Convert.ToDateTime(string.Format("{0:yyyy-MM-dd HH:mm:ss.fff}", reader["DTS"]));
                        LogInfo("[ " + Name + " ] --INF-- Last order DTS: " + orderStartDateTime.ToString(CultureInfo.InvariantCulture), logger);
                    }
